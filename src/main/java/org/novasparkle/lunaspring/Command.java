    ...
    public boolean onCommand(@NotNull CommandSender sender, org.bukkit.command.@NotNull Command command, @NotNull String s, @NotNull String[] args) {
        if (args.length == 1 && sender.hasPermission("lunaspring.reload")) {
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                sender.sendMessage(Objects.requireNonNull(LunaSpring.getINSTANCE().getConfig().getString("messages.reloaded")));
            }
            else if (args[0].equalsIgnoreCase("server-info")) {
                List<String> list = LunaSpring.getINSTANCE().getConfig().getStringList("messages.server-info");
